...
public class CameraMovement : MonoBehaviour
{
    ...
    void Start()
    {
        zPosition = -1*(LoadingBitmap.image.height * 2) - 2;//one unit past player
        transform.position = new Vector3(transform.position.x, transform.position.y, zPosition);
        Debug.Log(zPosition);
        thePlayer = transform.parent.gameObject;
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(transform.position, thePlayer.transform.position) == 1 )
        {
