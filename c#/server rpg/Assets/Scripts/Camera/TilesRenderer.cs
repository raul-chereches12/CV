...
public class TilesRenderer : MonoBehaviour
{
    ...
    private void ScanAround()
    {
        ...
        int starting_i = (int)(transform.position.x - transform.parent.position.x - 2 * transform.position.y - transform.parent.position.y + LoadingBitmap.image.height / 2) /LoadingBitmap.chunkSize;
        int starting_j = (int)(transform.position.x - transform.parent.position.x + 2 * transform.position.y - transform.parent.position.y + LoadingBitmap.image.height / 2 )/ LoadingBitmap.chunkSize;
        ...
        for (int i=starting_i;i< starting_i+Length; i++)
            for(int j=starting_j;j< starting_j+Width; j++)
            {
                ...
                if (!LoadingBitmap.chunksArray[i, j].active)
                {
                    LoadingBitmap.chunksArray[i, j].active = true;
                    LoadingBitmap.chunksArray[i, j].InstantiateChunk();
